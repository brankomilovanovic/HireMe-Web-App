...
class UserHandler extends BaseHandler
{
    ...
    public function create(Request $request)
    {
        try {
            ...
            if ($this->repository->get($params['username'], 'username')) {
                return $this->responseWithMessage('User with that username already exists!', 400);
            }

            if ($this->repository->get($params['email'], 'email')) {
            ...
            $this->handleCreateUserType($user, $params);
    ...
    public function handleCreateUserType(User $user, $params) {

        if($user->getUserType() === UserTypeEnum::COMPANY) {
            $entity = new UserCompany();

            if(isset($params['companyName'])) {
                $entity->setName($params['companyName']);
            }

            if(isset($params['companyAddress'])) {
                $entity->setAddress($params['companyAddress']);
            }

            $entity = $this->em->getRepository(UserCompany::class)->save($entity);
            $user->setUserCompany($entity);
        }

        if($user->getUserType() === UserTypeEnum::INDIVIDUAL) {
            $entity = new UserIndividual();

            if(isset($params['individualAddress'])) {
                $entity->setAddress($params['individualAddress']);
            }

            $entity = $this->em->getRepository(UserIndividual::class)->save($entity);
            $user->setUserIndividual($entity);
        }
    }
}
